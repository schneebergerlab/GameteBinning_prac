...
if(length(args) < 2) {
  ...
  cat("   USAGE: Rscript calculate_CO_landscape.R work_path CO_list.csv\n\n")
}else
{
  # parameters
  wd=args[[1]]
  ...
  options(scipen=9990)
  #
  pdf(paste("Suppl_Fig_Rowan_et_al_2019_slidwin_s1_real_only.pdf", sep=""), family="Helvetica", height=2.5, width=8.26772/2)
  par(ps=8)
  #
  m <- rbind(c(1,1,1),
             c(2,2,2) )
  ...
  for (chr in c(1,4))
  {
    ...
    ####
    legend("topright", pch = c(15, 15), col=c("orangered"), legend = c("Real"), border = NA, bty="n")
  }
  # close pdf
  dev.off()
  # output counts
