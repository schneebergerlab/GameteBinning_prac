/*  Given     
        a list of linkage map of ordered contigs -- from JoinMap/MSTMap       -- (X1:contigs of grouping+ordering),
        a list of .............. phased  contigs -- from JoinMap/MSTMap       -- (X2:contigs of grouping+phasing)        
        and         
    ...
    Note 4: on output, for each genetic map group, 
            ...
            consistently naming the same parents.
                                 ...
                                 by Hequan Sun, MPIPZ
    2020-01-02 15:47     updated by Hequan Sun, MPIPZ with del-like regions
    2020-02-11 13:28     updated by Hequan Sun, MPIPZ hap_low can be updated as hom_low according to cell support.
*/
#include            <map>
#include         <string>
#include         <vector>
...
int main(int argc, char* argv[])
{
    if(argc < 11)
    {
        // g++ asCaffolder_dip.cpp split_string.cpp -O3 -o asCaffolder_dip
    ...
    double startT= clock();
    string ofilename = "asCaffolder_dip.log";
    ofp_log.open(ofilename.c_str(), ios::out);
    if(!ofp_log.good())
    {
        cout << "   Error: cannot open a log file in the current folder. Stopped. " << endl;
        return 1; 
    }
    // step 0. initialize from the cmd line
    ofp_log << "Initialize input from cmd line: " << endl << endl;
    string fmap             = "";
    string fphase           = "";    
    string fmarker          = "";
    string fmarker_del_like = "";
    ...
    ofp_log << " Step 1: reading genetic map info on contig markers..." << endl;
    ...
    while(mapfitr != mapfitr_end)
    {
        ...
        ofp_log << endl;
        ofp_log << "   Info: reading genetic map info from " << this_gmfile << "..."    << endl;        
        vector<string>       gmContigOrder;
        map<string, string>  unmappedContig;                   
        if(!get_contig_genetic_map(this_gmfile,
                                   ...
                                   &unmappedContig))
        {
            ofp_log << "   Error: failed in reading " << this_gmfile << endl;
            return 1;
        }
        ofp_log << "   Info: reading genetic map info from " << this_gmfile << " done. " << endl;       
        //
        contig_genetic_map.insert(std::pair<string, vector<string> >(this_genetic_map_file, gmContigOrder));      
        //
        mapfitr ++;
    }
    ofp_log << " Step 1: reading genetic map info on contig markers done."    << endl  << endl;       
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////// step 2. get contig phasing
    ofp_log << " Step 2: reading phasing info on contig markers in linkage groups... " << endl;
    ...
    while(gpitr != gpitr_end)
    {
        ...
        ofp_log << "   Info: read linkage-phasing info from " << fileinfo[fileinfo.size()-1] << endl;    
        map<string, bool> ctgJoinMapPhasing;
        if(!get_contig_phase(this_jmfile, 
                             ...
                             &nflip))
        {
            ofp_log << "   Error: failed in reading " << this_jmfile << endl;
            return 1;
        }
        nflip_all += nflip;
        contig_phasing.insert(std::pair<string, map<string, bool> >(this_linkage_file, ctgJoinMapPhasing));
        ofp_log << "         phasing info on " << ctgJoinMapPhasing.size() << " contig markers collected. " << endl;
        //
        gpitr ++;
    }
    map<string, bool>::iterator phitr;
    map<string, bool>::iterator phitr_end;
    phitr     = ctgJoinMapPhasing_all.begin();
    phitr_end = ctgJoinMapPhasing_all.end();
    ofp_log << "   Check0: initial phasing info with snp-markers: " << endl;
    while(phitr != phitr_end)
    {
        ofp_log << "       : " << (*phitr).first << " with phase={" << (*phitr).second << "}" << endl;
        //
        phitr ++;
    }    
    ofp_log << "   Info: overall, phasing info on " << ctgJoinMapPhasing_all.size() << " contig markers collected." << endl;
    ofp_log << "   Info: overall, PM pattern of "   << nflip_all << " contig markers need flipping. "         << endl;
    ofp_log << " Step 2: reading phasing info on contig markers in linkage groups done. "               << endl << endl;   
    
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
    /////////////////////////////////////// step 3: match phased group and genetic map group
    ofp_log << " Step 3: matching the phased group of contigs with groups of contigs in genetic map... "       << endl;    
    ...
    if(!match_phasegroup_mapgroup(contig_genetic_map, 
                                  ...
                                  &matched_map_phase))
    {
        ofp_log << "   Error: cannot match phased contigs with those in genetic map. " << endl;
        return 1;
    }    
    if(1)
    {
        ofp_log << "   Info: phased group should be more complete than mapped regarding number of markers. "     << endl;    
        ...
        {
            ofp_log << "   Info: phased-group: "                 << (*checkitr).first  
                 << " <=paired with=> mapped/ordered-group: " << (*checkitr).second << endl;
            checkitr ++;
        }
    }
    ofp_log << " Step 3: matching the phased group of contigs with groups of contigs in genetic map done. "<<endl<< endl;    
         
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////// step 4. get contig PM patterns from asPollinator
    ofp_log << " Step 4: reading PM patterns on contig markers from asPollinator... "            << endl;    
    ...
    {
        ofp_log << "   Error: failed in reading file " << fmarker << endl;
        return 1;
    }
    ofp_log << " Step 4: reading PM patterns on contig markers from asPollinator done. " << endl << endl;    
    ...
    ofp_log << "   Info: " << contig_gmap_group.size() << " contig markers in genetic map - reminder. " << endl   << endl;
    ofp_log << " Step 4.x: reading PM patterns on del-like contig-regional markers from del_marker_genotyper... " << endl;
    map<string, map<unsigned long, DELMARKER> > del_like_marker_PM;
    if(!get_del_like_PMsequence(fmarker_del_like, &del_like_marker_PM))
    {
        ofp_log << "   Error: failed in reading file " << fmarker_del_like << endl;
        return 1;
    }
    ofp_log << "   Info: such del-like markers need to be inserted into the genetic map based on PM patterns later: \n"
    ...
    ofp_log << " Step 4.x: reading PM patterns on del-like contig-regional markers from del_marker_genotyper done. "
         << endl
         << endl;
    //
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
    ...
    ofp_log << " Step 5: completing genetic map with PM patterns of contig markers from asPollinator... "     << endl; 
    ofp_log << "         Note: for a missing marker x:X(PM-pattern), and candidate insertion sites y:Y and z:Z: "
         << endl
         << "               ins score  = 0;"                                                               << endl           
    ...
    {
        ofp_log << "   Error: failed in completing the genetic map. " << endl;
        return 1;
    }                    
    // check 1 20211128
    // map<string, bool>               ctgJoinMapPhasing_all; // map<contig_id, bool-flip>
    phitr     = ctgJoinMapPhasing_all.begin();
    phitr_end = ctgJoinMapPhasing_all.end();
    ofp_log << "   Check1: updated phasing info with snp-markers: " << endl;
    while(phitr != phitr_end)
    {
        ofp_log << "       : " << (*phitr).first << " with phase={" << (*phitr).second << "}" << endl;
        if( (*phitr).second == true)
        {
            nflip_all ++;
        }
        //
    ...
    ofp_log << "   Info: overall, phasing info on " << ctgJoinMapPhasing_all.size() << " contig markers collected." << endl;
    ofp_log << "   Info: overall, PM pattern of "   << nflip_all << " contig markers need flipping. "         << endl;                    
    ofp_log << " Step 5: completing genetic map with PM patterns on contig markers from asPollinator done. " 
         << endl << endl;    
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
    ...
    ofp_log << " Step 6: completing genetic map with PM patterns of del-like contig markers from del_marker_genotyper... "
         << endl;     
    ctgGroup.clear();
    ...
    // check 2 20211128
    // map<string, bool>               ctgJoinMapPhasing_all; // map<contig_id\tleft/right, bool-flip>
    phitr     = ctgJoinMapPhasing_all.begin();
    phitr_end = ctgJoinMapPhasing_all.end();
    ofp_log << "   Check2: updated phasing info with del-markers: " << endl;
    while(phitr != phitr_end)
    {
        ofp_log << "       : " << (*phitr).first << " with phase={" << (*phitr).second << "}" << endl;
        if( (*phitr).second == true)
    ...
    ofp_log << "   Info: overall, phasing info on " << ctgJoinMapPhasing_all.size() << " contig markers collected." << endl;
    ofp_log << "   Info: overall, PM pattern of "   << nflip_all << " contig markers need flipping. "         << endl;                    
    ofp_log << " Step 6: completing genetic map with PM patterns of del-like contig markers from del_marker_genotyper done."
         << endl << endl;
    //
    double finishT= clock();
    ofp_log << "   Time consumed: " << (finishT-startT)/1000000 << " seconds" << endl << endl;    
    return 0;
}
                                    ...
                                    string                                       outprefix)
{
    ...
    */ 
    ...
    if(!ofp.good())
    {
        ofp_log << "   Error: cannot open file to collected phased del-like markers: " << ophasedfile << endl;
        return false;
    }
    //
    ...
    while(delctgitr != delctgitr_end)
    {
        ...
        {        
            ...
            {
                ofp_log << "       : case 1or2 - contig of this del-like marker \"" << ss.str() << "\" NOT in genetic map: " << endl;
                   ...
                   ctgGroup.find(this_right_id) == ctgGroup.end())
                {
                    ...
                    ofp_log << "       : case 1 - also NOT in phased groups, "                     << endl;   
                    if(this_node.haphom.find("hom") != std::string::npos)
                    {
                        // case 1.1: hom 
                        ofp_log << "                  ";
                        ofp_log << "but its depth>=hom => no further genetic map completing can be done with it; "
                    ...
                    }else
                    {
                        ...
                        {                             
                            ...
                            {
                                ofp_log << "       : investigating genetic map " << (*gmfitr).first << endl;
                                ...
                                ofp_log << "       : score with M pattern = " << highest_score_flipped << " (flipped) "<< endl;
                            ...
                            {                         
                                ...
                                ofp_log << this_raw_pat_flipped     << "\t" << ss.str() << endl
                            ...
                            }else
                            {
                                ...
                                ofp_log << this_raw_pat             << "\t" << ss.str() << endl
                                     << contigMarkerPM[*ins_itr] << "\t" << *ins_itr << endl;                                
                            }
                            //
                            // update genetic map and related information
                            ofp_log << "       : inserting before " << *ins_itr
                            ...
                            ofp_log << "       : related PM phasing file: " << matched_map_phase[this_gmap_file_overall] 
                                 << ". " << endl << endl;                 
                ...
                }else
                {
                    ...
                    ofp_log << "       : case 2- but IN phased group "    << this_linkage_file 
                    ...
                    }else
                    {
                        // case 2.2: hap
                        ofp_log << "                  ";                        
                        ofp_log << "and it is hap => phasing it according PM similarity "
                             << "to " << this_ctg << " in group "             
                             << this_linkage_file
                             << " to decide genotype of pacbio reads. " 
                             << endl;
                        ofp_log << "                  note: if " << this_ctg << " does not have enough effective pollen; "
                             << "using other PM of other contigs in the same group. "
                             << endl;
                        //
                        ofp_log << "       : investigating genetic map " << this_gmap_file << endl;
                        // find insertion site of current marker in the genetic map
                        long highest_score;
                        bool rely_on_paired = false;
                        long highest_score_overall = 0;
                        ...
                        ofp_log << "       : inserting before " << *ins_itr
                             ...
                             << " phase={"                   << phase_assign << "}"
                             << endl;
                        ofp_log << "       : related linkage file: " << matched_map_phase[this_gmap_file] 
                             << ". " << endl << endl;
            ...
            }else
            {
                ...
                vector<string> this_gm_contigs = contig_genetic_map[this_gmap_file];                                   
                ofp_log << "       : case 3 - contig of this del-like marker \"" << ss.str() 
                ...
                }else
                {
                    ...
                    //                    
                    ofp_log << "                  Comp: investigating genetic map " << this_gmap_file << endl;
                    // find insertion site of current marker in the genetic map
                    ...
                    ofp_log << "                       : score with P pattern to " << this_left_id << ": " << highest_score << endl;
                    if(highest_score == highest_score_overall)
                    {
                        ofp_log << "                   Warning: a tie found for " << this_raw_mkr << " with score " 
                             << highest_score_overall << endl;
                    ...
                    ofp_log << "                       : score with M pattern to " << this_left_id << ": " << highest_score_flipped
                         << " (flipped) " << endl;                                        
                    if(highest_score_flipped == highest_score_overall)
                    {
                        ofp_log << "                   Warning: a tie found for " << this_raw_mkr << " with score " 
                    ...
                    // update genetic map and related information
                    ofp_log << "                  : no need to insert - " << this_left_id
                         << " with score "                  << highest_score_overall 
                         << " to  genetic map group "       << this_gmap_file
                         << " phase={"                      << phase_assign << "}"
                         << endl;
        ...
        ofp_log << "         Genetic map insertion cases summary for " << this_ctg << " related del-like: " << endl;
        ofp_log << "            checked " << this_del_like.size() << " del-like regions: " << endl;
        ...
        while(gmcitr != gmcitr_end)
        {
            ...
            //
            ofp_log << "               inserted into genetic map ";
            ofp_log << (*gmcitr).first             << "\t" 
            ...
            if((*gmcitr).second == max)
            {
                ...
                {                 
                    ofp_log << "            ";
                    ofp_log << "equal occurrence but grouping max-score " << gmap_score[(*gmcitr).first] 
                         << " with " << (*gmcitr).first
        ...
        if(max_cnt > 1)
        {
            ofp_log << "         current max observed more than once with same score. " << endl;
        }
        ofp_log << "         this contig would go to genetic map "                      << max_gmapfile 
        ...
        {
                ofp_log << "            ";
                ofp_log << "This del-like cannot be inserted into genetic map (or it is already in genetic map). "
                     << endl;
        }else
        {
            ...
            ofp_log << "            ";
            ofp_log << "Insert this contig " << this_ctg << " before " << max_marker
        ...
        ofp_log << "            Some more details about insertion of the del-like marker: " << endl;                
        ...
        while(tdlitr != tdlitr_end)
        {
            ...
            {
                ofp_log << "               ";                
                ofp_log << "This del-like " << ssdel.str() << " in hom regions - not phased - go to unmapped. " << endl;
            }else
            if((*tdlitr).second.status.find("unphased.hap.case1.2.1") != std::string::npos)
            {
                ofp_log << "               ";                
                ofp_log << "This del-like " << ssdel.str() << " in hap regions - not phased - go to unmapped. " << endl;
            }else  
            if((*tdlitr).second.status.find("phased.hap.case1.2.2")   != std::string::npos)
            {
                ...
                ofp_log << "               ";                                
                ofp_log << "Insert this del-like " << ssdel.str() << " before " << (*tdlitr).second.insmarker 
                     << " of " << max_gmapfile << " with original score " 
            ...
            {
                ofp_log << "               ";                
                ofp_log << "This del-like " << ssdel.str() << " in hom regions - not phased (but snp-marker phased). "<< endl;
            }else                        
            ...
            {
                ofp_log << "               ";
                ofp_log << "This del-like " << ssdel.str() << " in hom regions - not phased (but in genetic map). " << endl;
            }else            
            if((*tdlitr).second.status.find("phased.hap.case3.2")     != std::string::npos)
            {
                ofp_log << "               ";
                ofp_log << "This del-like " << ssdel.str() << " in hap regions - and phased (but in genetic map). " << endl;
            } 
            else ;            
            //
            tdlitr ++;
    ...
    ofp_log << "   Info: analyzed " << caseall_total << " del-like markers, including: "              << endl
    ...
    ofp_log << endl;
    ofp_log << "   Info: " << to_be_inserted_del_contig_id.size() 
         << " del-contigs need to be inserted in genetic map. " << endl;
    while(to_be_inserted_del_contig_id.size()>0 && tbitr != tbitr_end)
    {    
    ...
    if(1)
    {
        ...
        else if (ENOENT == errno)
        {
            ...
            if (dir_err == -1)
            {
                ofp_log << "   Error: cannot create directory " << tmpfolder << endl;
                return false;
            }
        }
        else;
        ...
        while(upd_mapfitr != upd_mapfitr_end)
        {
            ...
            ofp_log << "   Check: map "       << this_gm_file       << " update to " 
            ...
            string         this_gm_file_updPM_phase = tmpfolder + "/PM_phase_upd" + this_gm_file; // PM values of genetic map plus phase
            ofstream updofp;
            updofp.open(this_gm_file_upd.c_str(), ios::out);
            if(!updofp.good())
            {
                ofp_log << "   Error: cannot open file to collected updated genetic map: " 
            ...
            if(!updPMofp.good())
            {
                ofp_log << "   Error: cannot open file to collected updated PMs of genetic map: " 
                     << this_gm_file_updPM << endl;
                return false;
            }
            ofstream updPMofp_phase;
            updPMofp_phase.open(this_gm_file_updPM_phase.c_str(), ios::out);
            if(!updPMofp_phase.good())
            {
                ofp_log << "   Error: cannot open file to collected updated PMs and phases of genetic map: " 
                     << this_gm_file_updPM_phase << endl;
                return false;
            }            
            //       
            ...
            while(gmcitr != gmcitr_end)
            {
                ...
                // PM + phase 
                string marker_id  = *gmcitr;    // raw.......... as: contigid\tleft/right
                string marker_id2 = marker_id2; // to be updated as: contigid_le/ri
                vector<string> marker_info = split_string(marker_id, '\t');
                if(marker_id.find("le") != std::string::npos)
                {
                    marker_id2 = marker_info[0] + "_le";
                }else
                if(marker_id.find("ri") != std::string::npos)
                {
                    marker_id2 = marker_info[0] + "_ri";
                }else
                {
                    ofp_log << "   Error: unexpected marker id: " << marker_id << endl;
                    return false; 
                }
                string gt_tmp     = contigMarkerPM_updated[*gmcitr]; // PM of gt_tmp has been flipped according phase value.
                numbering ++;
                assert((*ctgJoinMapPhasing_all).find(marker_id) != (*ctgJoinMapPhasing_all).end());
                updPMofp_phase << numbering << "\t" << marker_id2 << "\t{"<< (*ctgJoinMapPhasing_all)[marker_id] << "}\t";
                for(int i = 0; i < gt_tmp.size(); i ++)
                {
                    if((*ctgJoinMapPhasing_all)[marker_id] == false)
                    {
                        // keep (non-flipped) "PM" pattern: non-flipped M->a, P->b
                        if(gt_tmp.substr(i, 1).compare("P") == 0)
                        {
                            updPMofp_phase << "b" << "\t";
                        }else
                        if(gt_tmp.substr(i, 1).compare("M") == 0)
                        {
                            updPMofp_phase << "a" << "\t";
                        }else
                        {
                    ...
                    {
                        // flip "PM" back to "MP": flipped P->a, M->b
                        if(gt_tmp.substr(i, 1).compare("M") == 0)
                        {
                            updPMofp_phase << "b" << "\t";
                        }else
        ...
        ofp_log << "   Info: final number of contig markers in genetic map: "  << total_gm_contig_markers << endl
                                    ...
                                    )
{
    ...
    *contigMarkerPM_updated    = contigMarkerPM;
    ...
    while(rawmkritr != rawmkritr_end)
    {
        ...
        {
            ofp_log << "   Info: this marker \"" << this_raw_mkr << "\" not in genetic map: " << endl;
            // level 2: check if current marker in phased group or not = phasing checking
            if((*ctgGroup_updated).find(this_raw_mkr) == (*ctgGroup_updated).end())
            {
                ...
                ofp_log << "       : this marker \""                 << this_raw_mkr 
                ...
                // update
                assert(matched_map_phase.find(this_gmap_file_overall) != matched_map_phase.end());
                ofp_log << "       : related PM phasing file: " << matched_map_phase[this_gmap_file_overall] << ". " << endl;                 
                (*ctgGroup_updated).insert(std::pair<string, string>(this_raw_mkr, 
                                                                     matched_map_phase[this_gmap_file_overall] ));
                //update
                if(phase_assign == true)
                {  
                    if((*contigMarkerPM_updated).find(this_raw_mkr) == (*contigMarkerPM_updated).end())
                    {
                        (*contigMarkerPM_updated).insert(std::pair<string, string>(this_raw_mkr, this_raw_pat_flipped));
                    }else
                    {
                        (*contigMarkerPM_updated)[this_raw_mkr] = this_raw_pat_flipped;
                    }
                }else
                {
                    if((*contigMarkerPM_updated).find(this_raw_mkr) == (*contigMarkerPM_updated).end())
                    ...
                    {
                        (*contigMarkerPM_updated)[this_raw_mkr] = this_raw_pat;
                    }                    
                }
                ofp_log << "   Info: new PM pattern collected for \"" << this_raw_mkr << "\"" << endl;
                ofp_log << "         " << (*contigMarkerPM_updated)[this_raw_mkr]     << "\tphase={" << phase_assign << "}"<< endl;
                ofp_log << "         " << this_raw_pat_flipped                        << "\tflipped" << endl;
                ofp_log << "         " << this_raw_pat                                << "\traw"     << endl;
            }else
            {
                ...
                string this_linkage_file       = (*ctgGroup_updated)[this_raw_mkr];
                string this_gmap_file          = matched_phase_map[this_linkage_file];
                vector<string> this_gm_contigs = contig_genetic_map[this_gmap_file];                
                ofp_log << "       : this marker "            << this_raw_mkr 
        ...
        {
            // ofp_log << "   Check: " << this_raw_mkr << " in map " << contig_gmap_group[this_raw_mkr] << endl;
        }
        // next
        rawmkritr ++;
    }   
    ...
    ofp_log << "   Info: final phasing status collected for " << (*ctgJoinMapPhasing_all).size() << " markers. " << endl;
    //
    return true;
}
//
double calculate_recomb_freq(string PMpat1, string PMpat2)
{
    ...
    //         << PMpat2 << endl;
    //
    ofp_log << "   Info: effectivep.=" << effectivep              << endl
                                    ...
                                    long*               highest_score)
{
    ...
    {
        // should never happen
        ofp_log << "   Error: unexpected contig id " << this_raw_mkr << endl;
    ...
    */
    ...
    //// if(highest_itr != this_gm_contigs.end())    
    {
        ...
        }else
        {
            ...
            {
                ofp_log << "       : a tie found for "        << this_raw_mkr 
...
long find_PM_pattern_match(string pm1, string pm2, bool scale)
{
    ...
    }else
    {
        ...
        m = pm2.size() - m; // P or M
        if(n > m && m >= pm1.size()/4) // why 1/4? at least ~50 effective markers...
        {
            n = m;
        }
        if(n == 0) 
...
bool get_contig_PMsequence(string mkrfile, map<string, bool> ctgJoinMapPhasing_all, map<string, string>* contigMarkerPM)
{
    ...
    if(!ifp.good())
    {
        ofp_log << "   Error: cannot open PM pattern file " << mkrfile << endl;
        return false;
    }
    ...
    ofp_log << "   Info: " << (*contigMarkerPM).size() << " contig marker PM pattens collected; "   << endl; 
    ofp_log << "         " << phased_corrected         << " flipped according to JoinMap phasing. " << endl;
    ifp.close();
    return true;
}            
//
                 ...
                 map<string, map<unsigned long, DELMARKER> >* del_like_marker_PM)
{
    ...
    if(!ifp.good())
    {
        ofp_log << "   Error: cannot open del-like PM pattern file: " << del_like_file << endl;
        return false;        
    }
    ofp_log << "   Info: PM will be re-determined with RPKM value - so it can differ from raw PM pattern. "  << endl;
    //
    ...
    while(ifp.good())
    {
        ...
        if(line.find("rpkm") != std::string::npos)
        {
            ...
            if(lineinfo.size() < 7)
            {
                ofp_log << "   Warning: skipped unexpected line with insufficient info at " << line << endl;
                continue;
            }
            del_like_num ++;
            DELMARKER this_node;            
    ...
    ifp.close();
    //
    ofp_log << "   Info: " << del_like_num  << " del-like markers collected from " 
         << (*del_like_marker_PM).size() << " contigs."
         << endl;
    ofp_log << "   Warning: " << update_del_marker_definition << " hap_low markers have been reset as hom_low "
                            ...
                            map<string, string>* unmappedContig)
{
    ...
    */
    ...
    if(!ifp.good())
    {
        ofp_log << "   Error: cannot open file " << gmfile << endl;
        return false;
    }
    ...
    while(ifp.good())
    {
        ...
        if(lineinfo.size()==1)
        {
            lineinfo = split_string(line, '\t');
            ofp_log << "   Warning: info of genetic map re-analyzed with tab separator. " << endl;
        }
        if(lineinfo.size()<2)
        {
            ofp_log << "   Warning: insufficient line info: " << line << endl;
            continue;
        }
        //
        ...
        {
            ofp_log << "    Error: unexpected contig format: "               << line << endl;
            ofp_log << "         : --expected contig format: contigxxxxxxxx_le/ri. " << endl;
            return false;
        }
        if(line.find("unmapped")!=std::string::npos)
        {
            (*unmappedContig).insert(std::pair<string, string>(thiscontig+"\t"+leri, "unmapped"));
            //ofp_log << "   Check: " << thiscontig+"\t"+leri << " inserted in unmapped list. " << endl;
        }else
        {
            GNMAP tmpmap;
            tmpmap.distance = atof(lineinfo[1].c_str()); // original labeled genetic distance value 
            tmpmap.group    = groupLabel;                // linkage group
            tmpmap.status   = "IniMapped";               // this is in map according to JoinMap
            ...
            // ofp_log << "   Check: "    << thiscontig+"\t"+leri 
            //      << " inserted in   mapped list: gmval=" 
            //      << tmpmap.distance << ",  lineinfo=" << lineinfo.size() << endl;            
        }
    }
    ofp_log << "   Info: in " << groupLabel << ", found " << (*unmappedContig).size() 
         << " unmapped contig markers;"  << endl;
    ofp_log << "   Info: in " << groupLabel << ", found " << (*gmContigOrder).size()  
         << "   mapped contig markers."  << endl;
    if((*unmappedContig).size() > 0)
    {
        ofp_log << "   Info: updating genetic maps based on paired-end marker info of a contig. " << endl;    
        ...
        while(unmapitr != unmapitr_end)
        {
            ...
            {
                //ofp_log << "   Check: pair-end marker of " << pairendctg << " found in genetic map for unmapped: " 
                //     << unmappedcontig                  << endl;
                // update mapped list
                if(ctginfo[1].compare("right")==0)
                {
                    ...
                    //ofp_log << "   Check: " << unmappedcontig << " inserted after mapped " << pairendctg << endl;
                }else
                {
                    (*gmContigOrder).insert(mapitr, unmappedcontig);
                    //ofp_log << "   Check: " << unmappedcontig << " inserted before mapped " << pairendctg << endl;                
                }
            ...
            {
                //ofp_log << "   Check: " << unmappedcontig << " with pair " << pairendctg << " not in map." << endl;
                unmapitr ++;
            }
        }
        ofp_log << "   Info: in "               << groupLabel                                                   << ", " 
        ...
        if((*unmappedContig).size() > 0)
        ofp_log << "   Info: remaining unmapped means both end-markers are not in original genetic map. "       << endl
...
string retrieve_contigid(string jmcontigid)          
{
    // jmcontigid: contigid_le or contigid_ri
    string thiscontig = "";
    thiscontig = jmcontigid.substr(0, jmcontigid.size()-3);
    //
    return thiscontig;
}
//
                      ...
                      int*                 nflip)
{
    ...
    if(!jmifp.good())
    {
        ofp_log << "   Error: cannot open file " << jmfile << endl;
        return false;
    }
    //
    ...
    */
    ...
    while(jmifp.good())
    {
        ...
        if(lineinfo.size()<3) 
        {
            ofp_log << "   warning: unexpected line: " << endl;
            continue;
        }
        //
        if(line.find("{0}")!=std::string::npos || line.find("{1}")!=std::string::npos)
        {
            string tigid  = lineinfo[1];                //    tig00004185_ri       
            ...
            //
            // ofp_log << "   Check: phasing contig key collected: " << thiskey << endl;
            // single group-wise
            ...
            if(tmpcitr != (*ctgJoinMapPhasing).end())
            {
                ...
                {
                    ofp_log << "   Warning1: left and right phasings differ on the same contig " << thiskey << endl;
                    ofp_log << "            need to update my code!! "                           << endl;
            ...
            if(tmpcitr != (*ctgJoinMapPhasing_all).end())
            {
                if((*tmpcitr).second != flip)
                {
                    ofp_log << "   Warning2: phasings differ on the same contig for "  << thiskey << endl;
                    ofp_log << "            need to update my code!! "                            << endl;
                    // this should only happens on mis-assembled contigs?
                }
            }else
            ...
            if(c2jmitr != (*ctgGroup).end())
            {
                ...
                {
                    ofp_log << "   Warning3: same contig found in different linkage groups? It can happen! " << endl;
                    //
                    (*c2jmitr).second += "#";
    ...
    //ofp_log << "   Info: PM pattern of " << (*nflip) << " contig markers need flipping. " << endl;
    jmifp.close();
    //    
    return true;
}
...
bool get_group_files(string listfiles, vector<string>* groupfiles, string gmapphasing)
{
    ...
    if(!ifp.good())
    {
        ofp_log << "   Error: cannot open file " << listfiles << endl;
        return false; 
    }
    while(ifp.good())
    {
        ...
        if(!tmpifp.good())
        {
            ofp_log << "   Warning: file link not valid, skipped: " << line << endl;
            
        }else
        {
            (*groupfiles).push_back(line);
            ofp_log << "   Info: " << gmapphasing << " file collected: " << line << endl;
            tmpifp.close();
        }
    }
    ifp.close();
    ...
    while (ic < argc)
    {
        ...
        if(optstr.compare("--map") == 0)
        {
            ...
            *fmap = (string)argv[ic];
            ...
            {
                fp.close();
                ofp_log << "   Info: genetic map file (=genetic ordering of contigs, e.g, by JoinMap) provided: "         
                     << *fmap << endl;
            }
            else
            {
                ofp_log << "   Error: cannot open linkage map file "      
                     << *fmap << endl;
                return false;
            }
        }
        ...
        if(optstr.compare("--marker") == 0)
        {
            ...
            *fmarker = (string)argv[ic];
            ...
            {
                fp.close();
                ofp_log << "   Info: contig marker file (=PM patterns at contig-paired-ends by snp markers) provided: "         
                     << *fmarker << endl;
            }
            else
            {
                ofp_log << "   Error: cannot open marker file "      
                     << *fmarker << endl;
                return false;
            }
        }
        ...
        if(optstr.compare("--marker-del-like") == 0)
        {
            ...
            *fmarker_del_like = (string)argv[ic];
            ...
            {
                fp.close();
                ofp_log << "   Info: del-like marker file (=PM patterns of regions without snp markers) provided: "         
                     << *fmarker_del_like << endl;
            }
            else
            {
                ofp_log << "   Error: cannot open del-like marker file "      
                     << *fmarker_del_like << endl;
                return false;
            }
        }
        ...
        if(optstr.compare("--hap2hom") == 0)
        {
            ...
            ofp_log << "   Info: hap_low region would be reset as hom_low if its cell support ratio is larger than "
        ...
        if(optstr.compare("--phase") == 0)
        {
            ...
            *fphase = (string)argv[ic];
            ...
            {
                fp.close();
                ofp_log << "   Info: contig phasing file (=grouping of contigs according to contig-end-markers) provided: "         
                     << *fphase << endl;
            }
            else
            {
                ofp_log << "   Error: cannot open contig phasing file "      
                     << *fphase << endl;
                return false;
            }
        }else
        if(optstr.compare("-o") == 0)
        {
            ...
            *outprefix = (string)argv[ic];
            ofp_log << "   Info: output files will be labeled with \""    
                 << *outprefix << "\"" << endl;
        }
        else              
        ...
        {
            *verbose = true;
            ofp_log << "   Info: verbose mode turned on. "    
                 << endl;
        }
        else
        {
            ofp_log << "   Warning: option "               << argv[ic] 
