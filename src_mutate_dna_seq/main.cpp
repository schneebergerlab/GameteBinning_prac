// Function: mutate given fasta sequences at a specified rate.
// Hequan Sun, MPIPZ.
...
int main(int argc, char* argv[])
{
    if(argc < 4)
    {
        cout << "\nFunction: mutate dna sequences in fasta with specified mutation rate (only SNPs)";
    ...
    // initialize a file where distribution of mutations will be recorded.
    fstream mkrfp;
    ss.str("");
    ss << argv[3] << ".rate" << pmrate << ".marker.txt";
    mkrfp.open((ss.str()).c_str(), ios::out);
    if(!mkrfp.is_open())
    {
        cout << "Cannot open file " << ss.str() << " to write mutations(/marker) info. Exited.\n";
        return 1;
    }
    cout << "\nMutations will be collected in file: " << ss.str() << "." << endl;
    // initialize a file where normalized of mutations will be recorded.
    fstream covfp;
    ss.str("");
    ss << argv[3] << ".rate" << pmrate << ".marker.cov.txt";
    covfp.open((ss.str()).c_str(), ios::out);
    if(!covfp.is_open())
    {
        cout << "Cannot open file " << ss.str() << " to write marker coverage info. Exited.\n";
        return 1;
    }
    cout << "\nMutations will be collected in file: " << ss.str() << "." << endl;        
    ...
    while(fp.good())
    {
        ...
        if(line[0]=='>')
        {
            ...
            for(size_t i=0; i<seq.length(); i++)
            {
                ...
                mkrfp << "sim\t" << line.substr(1) << "\t" << oripos+1 << "\t" << orich << "\t" << ch << endl;
                covfp << line.substr(1) << "\t" << oripos+1 << "\t" << rand()%100/(double)100/2+0.85 << endl;
                seq[oripos] = ch;
